											...
											<div class=" tab-menu-heading">
												<div class="tabs-menu1">
													...
													<ul class="nav panel-tabs main-nav-line">
														<li><a href="#tab4" class="nav-link active" data-toggle="tab">comptetions</a></li>
							...
							<div class="card-body">
								<div class="text-wrap">
									<div class="example">
                                        ...
                                        @foreach ($activeComptetions as $comptetion )
										@if($comptetion->user_id != Auth::user()->id)
                                        @if(checkComptetionAttendance($comptetion->id))
                                        <a class="list-group-item bg-success list-group-item-action flex-column align-items-start " href="/rank/{{ $comptetion->id }}">
											<div class="d-flex w-100 justify-content-between">
												<h3 class="mb-2 tx-14">{{ $comptetion->title }}</h3><strong>ends in : {{(strtotime($comptetion->end_date)
                                                     -  strtotime($comptetion->start_date))/(86400) }} days</strong>

                                                    </div>
											</a>
                                                @else
                                        <a class="list-group-item active list-group-item-action flex-column align-items-start " href="/comptetion/{{ $comptetion->id }}">
											<div class="d-flex w-100 justify-content-between">
												<h3 class="mb-2 tx-14">{{ $comptetion->title }}</h3><strong>ends in : {{(strtotime($comptetion->end_date)
                                                     -  strtotime($comptetion->start_date))/(86400) }} days</strong>
											</div>
											<p class="mb-1 tx-13">{{ $comptetion->description }}</small></a>
                                        @endif
                                        @endif
